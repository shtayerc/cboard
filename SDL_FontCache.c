...
static void
set_clip(FC_Target* dest, FC_Rect* rect) {
#ifdef FC_USE_SDL_GPU
    if (rect != NULL) {
...
static void
set_color(FC_Image* src, Uint8 r, Uint8 g, Uint8 b, Uint8 a) {
#ifdef FC_USE_SDL_GPU
...
static char*
new_concat(const char* a, const char* b) {
    // Create new buffer
...
static char*
replace_concat(char** a, const char* b) {
...
FC_Rect
FC_MakeRect(float x, float y, float w, float h) {
    FC_Rect r = {x, y, w, h};
    return r;
}

FC_Scale
FC_MakeScale(float x, float y) {
    FC_Scale s = {x, y};
...
SDL_Color
FC_MakeColor(Uint8 r, Uint8 g, Uint8 b, Uint8 a) {
    SDL_Color c = {r, g, b, a};
...
FC_Effect
FC_MakeEffect(FC_AlignEnum alignment, FC_Scale scale, SDL_Color color) {
    FC_Effect e;
...
FC_GlyphData
FC_MakeGlyphData(int cache_level, Sint16 x, Sint16 y, Uint16 w, Uint16 h) {
    FC_GlyphData gd;
...
static FC_GlyphData*
FC_MapInsert(FC_Map* map, Uint32 codepoint, FC_GlyphData glyph) {
    Uint32 index;
    FC_MapNode* node;
    if (map == NULL) {
        return NULL;
...
static FC_GlyphData*
FC_MapFind(FC_Map* map, Uint32 codepoint) {
    Uint32 index;
    FC_MapNode* node;
    if (map == NULL) {
        return NULL;
...
static_inline SDL_Surface*
FC_CreateSurface32(Uint32 width, Uint32 height) {
...
int
U8_charsize(const char* character) {
    if (character == NULL) {
        return 0;
    }
...
int
U8_charcpy(char* buffer, const char* source, int buffer_size) {
    int charsize;
...
const char*
U8_next(const char* string) {
    return string + U8_charsize(string);
}

int
U8_strinsert(char* string, int position, const char* source, int max_bytes) {
    int pos_u8char;
    int len;
    int add_len;
    int ulen;
...
void
U8_strdel(char* string, int position) {
...
static_inline FC_Rect
FC_RectUnion(FC_Rect A, FC_Rect B) {
    float x, x2, y, y2;
    x = FC_MIN(A.x, B.x);
    y = FC_MIN(A.y, B.y);
...
static_inline FC_Rect
FC_RectIntersect(FC_Rect A, FC_Rect B) {
    FC_Rect result;
...
FC_Rect
FC_DefaultRenderCallback(FC_Image* src, FC_Rect* srcrect, FC_Target* dest, float x, float y, float xscale,
                         float yscale) {
...
static FC_Rect (*fc_render_callback)(FC_Image* src, FC_Rect* srcrect, FC_Target* dest, float x, float y, float xscale,
                                     float yscale) = &FC_DefaultRenderCallback;

void
FC_SetRenderCallback(FC_Rect (*callback)(FC_Image* src, FC_Rect* srcrect, FC_Target* dest, float x, float y,
...
void
FC_GetUTF8FromCodepoint(char* result, Uint32 codepoint) {
    char a, b, c, d;
...
Uint32
FC_GetCodepointFromUTF8(const char** c, Uint8 advance_pointer) {
    Uint32 result = 0;
    const char* str;
    if (c == NULL || *c == NULL) {
        return 0;
...
void
FC_SetLoadingString(FC_Font* font, const char* string) {
    if (font == NULL) {
        return;
    }
...
unsigned int
FC_GetBufferSize(void) {
    return fc_buffer_size;
}

void
FC_SetBufferSize(unsigned int size) {
    free(fc_buffer);
    if (size > 0) {
        fc_buffer_size = size;
...
void
FC_SetTabWidth(unsigned int width_in_spaces) {
...
static Uint8
FC_GrowGlyphCache(FC_Font* font) {
    if (font == NULL) {
        return 0;
    }
#ifdef FC_USE_SDL_GPU
...
#else
    SDL_Texture* new_level = SDL_CreateTexture(font->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET,
    ...
    set_color(new_level, font->default_color.r, font->default_color.g, font->default_color.b,
              FC_GET_ALPHA(font->default_color));
#ifndef FC_USE_SDL_GPU
    {
        Uint8 r, g, b, a;
...
Uint8
FC_UploadGlyphCache(FC_Font* font, int cache_level, SDL_Surface* data_surface) {
    ...
    if (!fc_has_render_target_support) {
        ...
        new_level = SDL_CreateTexture(renderer, data_surface->format->format, SDL_TEXTUREACCESS_TARGET, data_surface->w,
                                      data_surface->h);
        SDL_SetTextureBlendMode(new_level, SDL_BLENDMODE_BLEND);
...
static FC_GlyphData*
FC_PackGlyphData(FC_Font* font, Uint32 codepoint, Uint16 width, Uint16 maxWidth, Uint16 maxHeight) {
    ...
    return FC_MapInsert(glyphs, codepoint,
                        FC_MakeGlyphData(last_glyph->cache_level, last_glyph->rect.x, last_glyph->rect.y,
                                         last_glyph->rect.w, last_glyph->rect.h));
}

FC_Image*
FC_GetGlyphCacheLevel(FC_Font* font, int cache_level) {
...
Uint8
FC_SetGlyphCacheLevel(FC_Font* font, int cache_level, FC_Image* cache_texture) {
...
#ifdef FC_USE_SDL_GPU
Uint8
FC_LoadFontFromTTF(FC_Font* font, TTF_Font* ttf, SDL_Color color)
#else
Uint8
FC_LoadFontFromTTF(FC_Font* font, SDL_Renderer* renderer, TTF_Font* ttf, SDL_Color color)
#endif
{
...
// Might as well check render target support here
...

    {
        ...
        for (; *source_string != '\0'; source_string = U8_next(source_string)) {
            ...
            packed = (FC_PackGlyphData(font, FC_GetCodepointFromUTF8(&buff_ptr, 0), glyph_surf->w,
                                       surfaces[num_surfaces - 1]->w, surfaces[num_surfaces - 1]->h)
                      != NULL);
            if (!packed) {
                int i = num_surfaces - 1;
                if (num_surfaces >= FC_LOAD_MAX_SURFACES) {
                    // Can't do any more!
                    FC_Log("SDL_FontCache error: Could not create enough cache surfaces to fit all of the loading "
                           "string!\n");
            ...
            if (packed
                || FC_PackGlyphData(font, FC_GetCodepointFromUTF8(&buff_ptr, 0), glyph_surf->w,
                                    surfaces[num_surfaces - 1]->w, surfaces[num_surfaces - 1]->h)
                       != NULL) {
...
#ifdef FC_USE_SDL_GPU
Uint8
FC_LoadFont(FC_Font* font, const char* filename_ttf, Uint32 pointSize, SDL_Color color, int style)
#else
Uint8
FC_LoadFont(FC_Font* font, FC_Target* renderer, const char* filename_ttf, Uint32 pointSize, SDL_Color color, int style)
#endif
{
    SDL_RWops* rwops;
...
#ifdef FC_USE_SDL_GPU
Uint8
FC_LoadFont_RW(FC_Font* font, SDL_RWops* file_rwops_ttf, Uint8 own_rwops, Uint32 pointSize, SDL_Color color, int style)
#else
Uint8
FC_LoadFont_RW(FC_Font* font, FC_Target* renderer, SDL_RWops* file_rwops_ttf, Uint8 own_rwops, Uint32 pointSize,
...
#ifndef FC_USE_SDL_GPU
void
FC_ResetFontFromRendererReset(FC_Font* font, SDL_Renderer* renderer, Uint32 evType) {
    TTF_Font* ttf;
    SDL_Color col;
    Uint8 owns_ttf;
    if (font == NULL) {
...
int
FC_GetNumCacheLevels(FC_Font* font) {
    return font->glyph_cache_count;
}

Uint8
FC_AddGlyphToCache(FC_Font* font, SDL_Surface* glyph_surface) {
...
unsigned int
FC_GetNumCodepoints(FC_Font* font) {
    FC_Map* glyphs;
    int i;
...
void
FC_GetCodepoints(FC_Font* font, Uint32* result) {
    FC_Map* glyphs;
    int i;
...
Uint8
FC_GetGlyphData(FC_Font* font, FC_GlyphData* result, Uint32 codepoint) {
...
FC_GlyphData*
FC_SetGlyphData(FC_Font* font, Uint32 codepoint, FC_GlyphData glyph_data) {
    return FC_MapInsert(font->glyphs, codepoint, glyph_data);
}

// Drawing
static FC_Rect
FC_RenderLeft(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text) {
    const char* c = text;
    FC_Rect srcRect;
    FC_Rect dstRect;
    ...
    for (; *c != '\0'; c++) {
        ...
        srcRect = glyph.rect;
#endif
        dstRect = fc_render_callback(FC_GetGlyphCacheLevel(font, glyph.cache_level), &srcRect, dest, destX, destY,
                                     scale.x, scale.y);
...
static void
set_color_for_all_caches(FC_Font* font, SDL_Color color) {
...
FC_Rect
FC_Draw(FC_Font* font, FC_Target* dest, float x, float y, const char* formatted_text, ...) {
...
void
FC_StringListFree(FC_StringList* node) {
    // Delete the nodes in order
    while (node != NULL) {
...
FC_StringList**
FC_StringListPushBack(FC_StringList** node, char* value, Uint8 copy) {
    if (node == NULL) {
        return NULL;
    }
...
FC_StringList**
FC_StringListPushBackBytes(FC_StringList** node, const char* data, int num_bytes) {
    if (node == NULL) {
        return node;
    }
...
static FC_StringList*
FC_Explode(const char* text, char delimiter) {
    FC_StringList* head;
...
static FC_StringList*
FC_ExplodeBreakingSpace(const char* text, FC_StringList** spaces) {
    FC_StringList* head;
    FC_StringList** node;
    const char* start;
    const char* end;
...
static FC_StringList*
FC_ExplodeAndKeep(const char* text, char delimiter) {
    FC_StringList* head;
    FC_StringList** node;
    const char* start;
    const char* end;
...
static void
FC_RenderAlign(FC_Font* font, FC_Target* dest, float x, float y, int width, FC_Scale scale, FC_AlignEnum align,
               const char* text) {
    switch (align) {
        case FC_ALIGN_LEFT: FC_RenderLeft(font, dest, x, y, scale, text); break;
        case FC_ALIGN_CENTER: FC_RenderCenter(font, dest, x + width / 2, y, scale, text); break;
        case FC_ALIGN_RIGHT: FC_RenderRight(font, dest, x + width, y, scale, text); break;
    }
}

static FC_StringList*
FC_GetBufferFitToColumn(FC_Font* font, int width, FC_Scale scale, Uint8 keep_newlines) {
    (void)scale; //just silence warning
...
static void
FC_DrawColumnFromBuffer(FC_Font* font, FC_Target* dest, FC_Rect box, int* total_height, FC_Scale scale,
                        FC_AlignEnum align) {
    int y = box.y;
    FC_StringList *ls, *iter;
...
FC_Rect
FC_DrawBox(FC_Font* font, FC_Target* dest, FC_Rect box, const char* formatted_text, ...) {
    Uint8 useClip;
    if (formatted_text == NULL || font == NULL) {
...
FC_Rect
FC_DrawBoxAlign(FC_Font* font, FC_Target* dest, FC_Rect box, FC_AlignEnum align, const char* formatted_text, ...) {
    Uint8 useClip;
    if (formatted_text == NULL || font == NULL) {
        return FC_MakeRect(box.x, box.y, 0, 0);
    }
...
FC_Rect
FC_DrawBoxScale(FC_Font* font, FC_Target* dest, FC_Rect box, FC_Scale scale, const char* formatted_text, ...) {
    Uint8 useClip;
    if (formatted_text == NULL || font == NULL) {
        return FC_MakeRect(box.x, box.y, 0, 0);
    }
...
FC_Rect
FC_DrawBoxColor(FC_Font* font, FC_Target* dest, FC_Rect box, SDL_Color color, const char* formatted_text, ...) {
    Uint8 useClip;
    if (formatted_text == NULL || font == NULL) {
        return FC_MakeRect(box.x, box.y, 0, 0);
    }
...
FC_Rect
FC_DrawBoxEffect(FC_Font* font, FC_Target* dest, FC_Rect box, FC_Effect effect, const char* formatted_text, ...) {
    Uint8 useClip;
    if (formatted_text == NULL || font == NULL) {
        return FC_MakeRect(box.x, box.y, 0, 0);
    }
...
FC_Rect
FC_DrawColumn(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, const char* formatted_text, ...) {
...
FC_Rect
FC_DrawColumnAlign(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_AlignEnum align,
    ...
    switch (align) {
        case FC_ALIGN_CENTER: box.x -= width / 2; break;
        case FC_ALIGN_RIGHT: box.x -= width; break;
        default: break;
    }
...
FC_Rect
FC_DrawColumnScale(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_Scale scale,
                   const char* formatted_text, ...) {
    FC_Rect box = {x, y, width, 0};
    int total_height;
...
FC_Rect
FC_DrawColumnColor(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, SDL_Color color,
                   const char* formatted_text, ...) {
    FC_Rect box = {x, y, width, 0};
    int total_height;
...
FC_Rect
FC_DrawColumnEffect(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_Effect effect,
                    const char* formatted_text, ...) {
    FC_Rect box = {x, y, width, 0};
    int total_height;
...
static FC_Rect
FC_RenderCenter(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text) {
    ...
    for (c = str; *c != '\0';) {
        if (*c == '\n') {
            *c = '\0';
            result = FC_RectUnion(
                FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str) / 2.0f, y, scale, str), result);
            *c = '\n';
            c++;
            str = c;
    ...
    result = FC_RectUnion(FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str) / 2.0f, y, scale, str),
                          result);
...
static FC_Rect
FC_RenderRight(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text) {
    FC_Rect result = {x, y, 0, 0};
    if (text == NULL || font == NULL) {
        return result;
    }
    ...
    for (c = str; *c != '\0';) {
        if (*c == '\n') {
            *c = '\0';
            result = FC_RectUnion(FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str), y, scale, str),
                                  result);
            *c = '\n';
            c++;
            str = c;
...
FC_Rect
FC_DrawScale(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) {
        return FC_MakeRect(x, y, 0, 0);
    }
...
FC_Rect
FC_DrawAlign(FC_Font* font, FC_Target* dest, float x, float y, FC_AlignEnum align, const char* formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) {
        return FC_MakeRect(x, y, 0, 0);
    }
    ...
    FC_Rect result;
    switch (align) {
        case FC_ALIGN_LEFT: result = FC_RenderLeft(font, dest, x, y, FC_MakeScale(1, 1), fc_buffer); break;
        case FC_ALIGN_CENTER: result = FC_RenderCenter(font, dest, x, y, FC_MakeScale(1, 1), fc_buffer); break;
        case FC_ALIGN_RIGHT: result = FC_RenderRight(font, dest, x, y, FC_MakeScale(1, 1), fc_buffer); break;
        default: result = FC_MakeRect(x, y, 0, 0); break;
    }
...
FC_Rect
FC_DrawColor(FC_Font* font, FC_Target* dest, float x, float y, SDL_Color color, const char* formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) {
        return FC_MakeRect(x, y, 0, 0);
    }
...
FC_Rect
FC_DrawEffect(FC_Font* font, FC_Target* dest, float x, float y, FC_Effect effect, const char* formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) {
        return FC_MakeRect(x, y, 0, 0);
    }
    ...
    FC_Rect result;
    switch (effect.alignment) {
        case FC_ALIGN_LEFT: result = FC_RenderLeft(font, dest, x, y, effect.scale, fc_buffer); break;
        case FC_ALIGN_CENTER: result = FC_RenderCenter(font, dest, x, y, effect.scale, fc_buffer); break;
        case FC_ALIGN_RIGHT: result = FC_RenderRight(font, dest, x, y, effect.scale, fc_buffer); break;
        default: result = FC_MakeRect(x, y, 0, 0); break;
    }
...
Uint16
FC_GetHeight(FC_Font* font, const char* formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) {
        return 0;
    }
...
Uint16
FC_GetWidth(FC_Font* font, const char* formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) {
        return 0;
    }
...
FC_Rect
FC_GetCharacterOffset(FC_Font* font, Uint16 position_index, int column_width, const char* formatted_text, ...) {
...
Uint16
FC_GetColumnHeight(FC_Font* font, Uint16 width, const char* formatted_text, ...) {
    int y = 0;
...
static int
FC_GetAscentFromCodepoint(FC_Font* font, Uint32 codepoint) {
    FC_GlyphData glyph;
...
static int
FC_GetDescentFromCodepoint(FC_Font* font, Uint32 codepoint) {
    FC_GlyphData glyph;
...
int
FC_GetAscent(FC_Font* font, const char* formatted_text, ...) {
    Uint32 codepoint;
    int max, ascent;
    const char* c;
...
int
FC_GetDescent(FC_Font* font, const char* formatted_text, ...) {
    Uint32 codepoint;
    int max, descent;
    const char* c;
...
int
FC_GetLineSpacing(FC_Font* font) {
    if (font == NULL) {
        return 0;
    }
...
SDL_Color
FC_GetDefaultColor(FC_Font* font) {
    if (font == NULL) {
...
FC_Rect
FC_GetBounds(FC_Font* font, float x, float y, FC_AlignEnum align, FC_Scale scale, const char* formatted_text, ...) {
    FC_Rect result = {x, y, 0, 0};
    ...
    switch (align) {
        case FC_ALIGN_LEFT: break;
        case FC_ALIGN_CENTER: result.x -= result.w / 2; break;
        case FC_ALIGN_RIGHT: result.x -= result.w; break;
        default: break;
    }
...
Uint8
FC_InRect(float x, float y, FC_Rect input_rect) {
    return (input_rect.x <= x && x <= input_rect.x + input_rect.w && input_rect.y <= y
            && y <= input_rect.y + input_rect.h);
}

// TODO: Make it work with alignment
Uint16
FC_GetPositionFromOffset(FC_Font* font, float x, float y, int column_width, FC_AlignEnum align,
                         const char* formatted_text, ...) {
    (void)align; //just silence warning
    FC_StringList *ls, *iter;
    Uint8 done = 0;
...
int
FC_GetWrappedText(FC_Font* font, char* result, int max_result_size, Uint16 width, const char* formatted_text, ...) {
    FC_StringList *ls, *iter;
...
void
FC_SetFilterMode(FC_Font* font, FC_FilterEnum filter) {
    if (font == NULL) {
        return;
    }
...
void
FC_SetSpacing(FC_Font* font, int LetterSpacing) {
    if (font == NULL) {
        return;
    }
...
void
FC_SetLineSpacing(FC_Font* font, int LineSpacing) {
    if (font == NULL) {
        return;
    }
...
void
FC_SetDefaultColor(FC_Font* font, SDL_Color color) {
    if (font == NULL) {
        return;
    }

...
machine_write(void* p) {
    ...
    if (uci_list != NULL) {
        for (i = 0; uci_list[i] != NULL; i++) {
            WriteFile(mc->fd_input, uci_list[i], strlen(uci_list[i]), &len, NULL);
...
machine_start(WindowData* data, int index) {
    ...
    DuplicateHandle(GetCurrentProcess(), mc->fd_output, GetCurrentProcess(), NULL, 0, 0, DUPLICATE_SAME_ACCESS);

    DuplicateHandle(GetCurrentProcess(), mc->fd_input, GetCurrentProcess(), NULL, 0, 0, DUPLICATE_SAME_ACCESS);

    CreateProcess(NULL, data->conf.machine_cmd_list[index][0], NULL, NULL, 1,

...
explorer_start(WindowData* data, int index) {
    ...
    DuplicateHandle(GetCurrentProcess(), e->fd_output, GetCurrentProcess(), NULL, 0, 0, DUPLICATE_SAME_ACCESS);

    DuplicateHandle(GetCurrentProcess(), e->fd_input, GetCurrentProcess(), NULL, 0, 0, DUPLICATE_SAME_ACCESS);

    CreateProcess(NULL, data->conf.explorer_exe_list[index], NULL, NULL, 1, CREATE_NEW_PROCESS_GROUP | CREATE_NO_WINDOW,
                  NULL, NULL, &si, &pi);
    ...
    SDL_Thread* thread = SDL_CreateThread(machine_write, NULL, (void*)data);
    SDL_DetachThread(thread);
    return 1;
}

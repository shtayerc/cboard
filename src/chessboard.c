...
rotation_toggle(WindowData* data) {
    data->rotation = data->rotation == RotationWhite ? RotationBlack : RotationWhite;
}
...
square_rotation(WindowData* data, Square sq) {
    ...
    return filerank2square(rotation_convert(data, file), rotation_convert(data, rank));
}
...
promotion_selected_piece(WindowData* data, Square sq, Color color) {
    int piece[] = {((color == White) ? WhiteQueen : BlackQueen), ((color == White) ? WhiteKnight : BlackKnight),
                   ((color == White) ? WhiteRook : BlackRook), ((color == White) ? WhiteBishop : BlackBishop)};
...
chessboard_focus_present(WindowData* data, Square src, Square dst, Piece prom_piece) {
    ...
    if (!game_move_is_last(&data->game)) {
        m = &data->game.line_current->move_list[data->game.line_current->move_current + 1];
    ...
    for (i = 0; i < m->variation_count; i++) {
        ...
        if (m->variation_list[i]->move_list[1].src == src && m->variation_list[i]->move_list[1].dst == dst
...
void
chessboard_move_do(WindowData* data, Square src, Square dst, Piece prom_piece, Status status) {
    if (game_move_is_present(&data->game, src, dst, prom_piece)) {
        chessboard_focus_present(data, src, dst, prom_piece);
        handle_position_change(data);
        return;
